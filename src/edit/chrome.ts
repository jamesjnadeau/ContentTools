 ...
 *
 * Four states, and each one is somewhere a person can genuinely be left:
 * a config that does not parse, a page that maps to no entry, an entry
 * whose body cannot be found, and an entry ready to edit.
 */
export type BarState =
    ...
    /** Everything the editor needs, found. */
    | {
        readonly kind: 'ready';
        ...
        case 'ready':
            return {
                ...
                /* Naming the ELEMENT, not just the selector, is the whole
                   point of this state. The editor replaces that element's
                ...
                hint: `Editing ${describeElement(state.body)}, `
                    + `matched by ${state.selector}.`
            };
    }
}
