 ...
 */
ContentEdit.Node = class Node {
    declare _bindings: any;
    declare _modified: any;
    declare _parent: any;
    ...
    trigger(eventName, ...args) {
        ...
        return (() => {
            const result = [];
            for (var callback of Array.from<any>(this._bindings[eventName])) {
                if (!callback) {
                    continue;
                }
                result.push(callback.call(this, ...Array.from<any>(args)));
            }
            return result;
        })();
    }
    ...
    taint() {
        ...
        for (var parent of Array.from<any>(this.parents())) {
    ...
    next() {
        ...
        // Duck-typed: only NodeCollection subclasses have children.
        const self = this as unknown as {children?: any[]};
        ...
        // ancestor for one.
        for (var node of Array.from<any>([this].concat(this.parents()))) {
    ...
    nextContent() {
        ...
        // `ContentEdit.Text`).
        return this.nextWithTest(node => node.content !== undefined);
    }
    ...
    previousContent() {
        ...
        // `ContentEdit.Text`).
        let node;
        return node = this.previousWithTest(node => node.content !== undefined);
    }
    ...
    static extend(cls) {
        ...
        // Two CoffeeScript-vs-ES-class differences bite here, and between them
        ...
        for (key of Object.getOwnPropertyNames(cls)) {
            ...
            if (Array.from<any>('__super__').includes(key)) {
                continue;
            }
            this.prototype[key] = value;
        }
...
ContentEdit.NodeCollection = class NodeCollection extends ContentEdit.Node {
    ...
    commit() {
        ...
        for (var descendant of Array.from<any>(this.descendants())) {
...
let Cls$bases: any = (ContentEdit.Element = class Element extends ContentEdit.Node {
    declare _attributes: any;
    declare _behaviours: any;
    declare _domElement: any;
    declare _domEventHandlers: any;
    declare _ieMouseDownEchoed: any;
    ...
    clone() {
        ...
        return (this.constructor as unknown as Record<string, any>).fromDOMElement(wrapper.children[0]);
    }
    ...
    drop(element, placement) {
        ...
        if (element) {
            ...
            if ((this.constructor as unknown as Record<string, any>).droppers[element.type()]) {
                (this.constructor as unknown as Record<string, any>).droppers[element.type()](
                    this,
                    element,
                    placement
                    );
    ...
    hasCSSClass(className) {
        ...
        if (this.attr('class')) {
            ...
            const classNames = (Array.from<any>(this.attr('class').split(' ')));
    ...
    merge(element) {
        ...
        if ((this.constructor as unknown as Record<string, any>).mergers[element.type()]) {
            return (this.constructor as unknown as Record<string, any>).mergers[element.type()](element, this);
    ...
    _onOver(ev) {
        ...
        if (!(this.constructor as unknown as Record<string, any>).droppers[dragging.type()] 
                && !dragging.constructor.droppers[this.type()]) {
            return;
        }
    ...
    static getDOMElementAttributes(domElement) {
        ...
        const attributes = {};
        for (var attribute of Array.from<any>(domElement.attributes)) {
    ...
    static _dropBoth(element, target, placement) {
        ...
        if (element.a) {
            ...
            if (element.a['class']) {
                const aClassNames = [];
                for (var className of Array.from<any>(element.a['class'].split(' '))) {
...
Cls$bases = (ContentEdit.ElementCollection = class ElementCollection extends ContentEdit.Element {
    ...
    constructor(tagName, attributes) {
        ...
        // CoffeeScript's mixin idiom `NodeCollection::constructor.call(this)`
    ...
    html(indent?) {
        // Return a HTML string for the node
        if (indent == null) { indent = ''; }
        const children = (Array.from<any>(this.children).map((c) => c.html(indent + ContentEdit.INDENT)));
    ...
    mount() {
        ...
        return Array.from<any>(this.children).map((child) =>
            child.mount());
    }
...
ContentEdit.ResizableElement = class ResizableElement extends ContentEdit.Element {
    declare _aspectRatio: any;
    declare _domSizeInfoElement: any;
    ...
    _getResizeCorner(x, y) {
        ...
        [x, y] = Array.from<any>([x - rect.left, y - rect.top]);
