 ...
 */
let Cls$lists: any = (ContentEdit.List = class List extends ContentEdit.ElementCollection {
...
ContentEdit.ListItem = class ListItem extends ContentEdit.ElementCollection {
    ...
    unindent() {
        ...
        if (grandParent.type() === 'ListItem') {
            ...
            for (sibling of Array.from<any>(siblings)) {
            ...
            // Middle - split the parent list and insert the element between
                ...
                if (this.list()) {
                    // NOTE: `slice` used to create a copy for safe iteration
                    // over a changing list.
                    for (child of Array.from<any>(this.list().children.slice())) {
                        child.parent().detach(child);
                        list.attach(child);
                    }
                }
    ...
    static fromDOMElement(domElement) {
        ...
        for (var childNode of Array.from<any>(domElement.childNodes)) {
            if (childNode.nodeType === 1) { // ELEMENT_NODE
