...
import type {ContentSelectNamespace} from '../../src/core/namespaces.js';
...
const ContentSelect = {} as unknown as ContentSelectNamespace;
...
ContentSelect.Range = class Range {
    ...
    select(element) {
        ...
        const [startNode, startOffset] = Array.from<any>(_getChildNodeAndOffset(element, this._from));
        const [endNode, endOffset] = Array.from<any>(_getChildNodeAndOffset(element, this._to));
    ...
    static query(element) {
        ...
        const [startNode, startOffset, endNode, endOffset] = Array.from<any>(_getNodeRange(
            element,
            docRange
            ));
...
var _getChildNodeAndOffset = function(parentNode, parentOffset) {
    ...
    const childStack = ((() => {
        const result = [];
        for (n of Array.from<any>(parentNode.childNodes)) {             result.push(n);
        }
        return result;
    })());
    ...
    while (childStack.length > 0) {
        ...
        switch (childNode.nodeType) {
            ...
            case Node.ELEMENT_NODE:
                ...
                if ((needle = childNode.nodeName.toLowerCase(), Array.from<any>(SELF_CLOSING_NODE_NAMES).includes(needle))) {
                    if (childOffset === 0) {
                        return [childNode, 0];
                    } else {
                ...
                // the stack to be processed.
                    ...
                    if (childNode.childNodes) {
                        Array.prototype.unshift.apply(
                            ...
                            ((() => {
                            const result1 = [];
                            for (n of Array.from<any>(childNode.childNodes)) {                                 result1.push(n);
                            }
                            return result1;
                        })())
                            );
...
var _getOffsetOfChildNode = function(parentNode, childNode) {
    ...
    while (childStack.length > 0) {
        ...
        if (otherChildNode === childNode) {
            var needle;
            if ((needle = otherChildNode.nodeName.toLowerCase(), Array.from<any>(SELF_CLOSING_NODE_NAMES).includes(needle))) {
                return offset + 1;
            }
            return offset;
        }
        ...
        switch (otherChildNode.nodeType) {
            ...
            case Node.ELEMENT_NODE:
                ...
                // Inline block elements count as a single character
                if ((needle1 = otherChildNode.nodeName.toLowerCase(), Array.from<any>(SELF_CLOSING_NODE_NAMES).includes(needle1))) {
                    offset += 1;
                ...
                // the stack to be processed.
                    ...
                    if (otherChildNode.childNodes) {
                        Array.prototype.unshift.apply(
                            ...
                            ((() => {
                            const result1 = [];
                            for (n of Array.from<any>(otherChildNode.childNodes)) {                                 result1.push(n);
                            }
                            return result1;
                        })())
                            );
...
var _getNodeRange = function(element, docRange) {
    ...
    if (startNode === element) {
        ...
        for (i = 0; i < childNodes.length; i++) {
            ...
            if (Array.from<any>(SELF_CLOSING_NODE_NAMES).includes(startNode.nodeName.toLowerCase)) {
                startOffset = 1;
            }
