 ...
 */
const Cls$strings = (HTMLString.String = class String {    declare _preserveWhitespace: any;
    declare characters: any;

    static declare _parser: any;
    ...
    isWhitespace() {
        ...
        for (var c of Array.from<any>(this.characters)) {
            if (!c.isWhitespace()) {
                return false;
            }
        }
    ...
    concat(...args) {
        ...
        // Concat the strings
        const newString = this.copy();
        for (var string of Array.from<any>(strings)) {
            ...
            if (inheritFormat && newString.length()) {
                ...
                for (c of Array.from<any>(tail.characters)) {
    ...
    html() {
        ...
        for (var c of Array.from<any>(this.characters)) {
            ...
            var head;
            closingTags = [];
            for (var openTag of Array.from<any>(openTags.slice().reverse())) {
                closingTags.push(openTag);
                if (!c.hasTags(openTag)) {
                    for (var closingTag of Array.from<any>(closingTags)) {
            ...
            // Open tags
            for (tag of Array.from<any>(c._tags)) {
        ...
        for (tag of Array.from<any>(openTags.reverse())) {
            html += tag.tail();
        }
    ...
    insert(index, substring, inheritFormat) {
        ...
        if (inheritFormat && (index > 0)) {
            ...
            for (c of Array.from<any>(middle.characters)) {
                c.addTags.apply(c, inheritedTags);
            }
        }
    ...
    optimize() {
        ...
        for (c of Array.from<any>(this.characters.slice().reverse())) {
            c._runLengthMap = {};
            ...
            for (tag of Array.from<any>(openTags)) {
                var head = tag.head();
        ...
        return (() => {
            const result = [];
            for (c of Array.from<any>(this.characters)) {
            ...
            // which case there's no need to apply a re-order.
                ...
                for (tag of Array.from<any>(c._tags.slice())) {
                    if (!tag.selfClosing()) {
                        c.removeTags(tag);
                    }
                }
                result.push(c.addTags.apply(c, (Array.from<any>(runLengths).map((t) => t[0]))));
            }
            return result;
        })();
    }
    ...
    slice(from, to?) {
        ...
        newString.characters = (Array.from<any>(this.characters.slice(from, to)).map((c) => c.copy()));
        return newString;
    }
    ...
    text() {
        ...
        return (this.constructor as unknown as Record<string, any>).decode(text);
    }

    toLowerCase() {
        // Return a copy of the string converted to lower case
        const newString = this.copy();
        for (var c of Array.from<any>(newString.characters)) {
            if (c._c.length === 1) {
    ...
    trim() {
        ...
        newString.characters = ((() => {
            const result = [];
            for (c of Array.from<any>(this.characters.slice(from, +to + 1 || undefined))) {                 result.push(c.copy());
            }
            return result;
        })());
    ...
    copy() {
        ...
        stringCopy.characters = (Array.from<any>(this.characters).map((c) => c.copy()));
        return stringCopy;
    }
    ...
    static join(separator, strings) {
        ...
        for (var s of Array.from<any>(strings)) {
...
class _Parser {
    declare _preserveWhitespace: any;
    declare attributeName: any;
    declare attributeValue: any;
    declare attributes: any;
    declare entity: any;
    declare fsm: any;
    declare head: any;
    declare selfClosed: any;
    declare selfClosing: any;
    declare string: any;
    declare tagName: any;
    declare tags: any;
    ...
    constructor() {
        ...
        // UPSTREAM BUG, preserved. The CoffeeScript read
        //   @fsm.addTransitions ALPHA_NUMERIC_CHARS.concat ['&'], ATTR_DELIM, ...
        // and CoffeeScript's implicit-call syntax bound EVERY following
        // argument to `concat`, not to `addTransitions`. So concat receives
        // two state constants and a callback as array elements, and
        // addTransitions is called with a single argument instead of four --
        // meaning this transition has never been registered as intended.
        // Faithful to the shipped behaviour; fixing it is a parser change
        // that needs verifying on its own.
        this.fsm.addTransitions((ALPHA_NUMERIC_CHARS as any[]).concat(['&'], ATTR_DELIM, ATTR_VALUE_NO_DELIM, function() {
            return this._back();
        })
        );
    ...
    _pushTag() {
        ...
        if (tag.selfClosing()) {
            ...
            // close it.
            if (!this.selfClosed && Array.from<any>(HTMLString.Tag.SELF_CLOSING).includes(this.tagName)) {
                this.fsm.reset();
            }
        }
